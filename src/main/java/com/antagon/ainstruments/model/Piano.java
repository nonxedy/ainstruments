...
public class Piano extends Instrument {
    ...
    private static final String SOUND_BASE_NAME = "piano";
    
    /**
     * Create a new piano for a player.
     * 
     * @param player The player who will use this piano
     */
    public Piano(Player player) {
        super(player);
    }
    ...
    private void playNoteWithVoiceChat(int note) {
        ...
        try {
            ...
            // We would need to use the actual API methods to play the sound
            // based on the API documentation and available classes
            
            plugin.getLogger().info("Playing note: " + note + " on piano for player " + player.getName() + " with VoiceChat API");
            
            // Example of the kind of code that would be here:
            // UUID playerUuid = player.getUniqueId();
            // StaticAudioChannel channel = api.createStaticAudioChannel(playerUuid);
            // AudioPlayer audioPlayer = api.createAudioPlayer(
            //     api.createAudioSource(getClass().getResourceAsStream("/sounds/" + soundFileName)),
            //     channel);
            // audioPlayer.play();
        } catch (Exception e) {
    ...
    private void playNoteWithMinecraftSound(int note) {
        try {
            ...
            plugin.getLogger().info("Playing note: " + note + " (pitch: " + pitch + ") on piano for player " + player.getName());
        } catch (Exception e) {
