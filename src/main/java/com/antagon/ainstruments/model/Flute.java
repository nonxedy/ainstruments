...
/**
 * Flute instrument implementation.
 */
public class Flute extends Instrument {

    private static final String SOUND_BASE_NAME = "flute";
    
    /**
     * Create a new flute for a player.
     * 
     * @param player The player who will use this flute
     */
    public Flute(Player player) {
        super(player);
    }
    ...
    private void playNoteWithVoiceChat(int note) {
        ...
        try {
            ...
            plugin.getLogger().info("Playing note: " + note + " on flute for player " + player.getName() + " with VoiceChat API");
        } catch (Exception e) {
    ...
    private void playNoteWithMinecraftSound(int note) {
        try {
            ...
            player.getWorld().playSound(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_FLUTE, 1.0f, pitch);
            plugin.getLogger().info("Playing note: " + note + " (pitch: " + pitch + ") on flute for player " + player.getName());
        } catch (Exception e) {
